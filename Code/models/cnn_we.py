...
from Code.utils.Classifications import embedding_index, model_cnn, metrics_class, glove_embedding
...
for df_name in ls_df:
    ...
    for emb_dim in ls_emb_dim:
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            model.fit(X_train_e, y_train_e, validation_data=(X_test_e, y_test_e), epochs=15, batch_size=50,
                      callbacks=[es, mc],
                      verbose=0)
            ...
            # Output
            dict_names = {'Dataframe': df_name, 'Classificatore': f'CNN-{emb_dim}', 'Modello': 'CNN(WE)'}
            df_ris = pd.DataFrame({**dict_names, **dict_metrics}, index=[0])
            df_temp = pd.concat([df_temp, df_ris])

            # export
            df_temp.to_csv(f"{path_data_out}/CNN(WE)_{df_name}_{emb_dim}.csv", index=False)
