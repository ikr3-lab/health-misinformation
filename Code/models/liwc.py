...
from Code.data_enrichment.liwc_features import extract_liwc_features
...
if extract_liwc:
    # Extract LIWC feature
    extract_liwc_features(absolute_path)
    path_data = f"{absolute_path}Data/data_liwc"
else:
...
for df_name in ls_df:
    ...
    y_col = 'class'
    if extract_liwc:
        x_col = [col for col in df.columns if col not in ['class', 'news_id']]
    else:
        x_col = include_features(liwc=True)
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        for clf_name, clf in dict_classifiers.items():
            ...
            # Output
            dict_names = {'Dataframe': df_name, 'Classificatore': clf_name, 'Modello': 'LIWC'}
            df_ris = pd.DataFrame({**dict_names, **dict_metrics}, index=[0])
            df_temp = pd.concat([df_temp, df_ris])

        # export
        df_temp.to_csv(f"{path_data_out}/LIWC_{df_name}.csv", index=False)
