...
from Code.utils.Classifications import metrics_class, glove_embedding, \
    model_cnn_all_features, include_features, feature_selection_classification
...
for df_name in ls_df:
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        for emb_dim in ls_emb_dim:
            ...
            model = model_cnn_all_features(X_train.shape[1], emb_layer)
            ...
            model.fit([X_train_e, X_train], y_train_e, validation_data=([X_test_e, X_test], y_test_e), batch_size=50,
            ...
            # Output
            dict_names = {'Dataframe': df_name, 'Classificatore': f'CNN-all-{emb_dim}', 'Modello': 'CNN(WE+all)'}
            df_ris = pd.DataFrame({**dict_names, **dict_metrics}, index=[0])
            df_temp = pd.concat([df_temp, df_ris])

    # export
    df_temp.to_csv(f"{path_data_out}/CNN(WE+all)_{df_name}.csv", index=False)
